...
def define_model(D_inverse_pl, A_tilde_pl, X_pl, Y_pl, node_size_pl, is_train, initial_channels, learning_rate, nodes_size_list_train, nodes_size_list_test, debug, top_k):
    ...
    return train_op, global_step, loss, pre_y, pos_score

def train_demo(X_train, D_inverse_train, A_tilde_train, Y_train, nodes_size_list_train, nodes_size_list_test, initial_channels, learning_rate, epoch, top_k, debug=False):
    ...
    train_op, global_step, loss, pre_y, pos_score = define_model(D_inverse_pl, A_tilde_pl, X_pl, Y_pl, node_size_pl, is_train, initial_channels, learning_rate, nodes_size_list_train, nodes_size_list_test, debug, top_k)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epoch):
            ...
            print("After %5s epoch, training acc %f." % (epoch, train_acc))    
...
def predict_test_data(model, X_test, D_inverse_test, A_tilde_test, Y_test, nodes_size_list_test):
    ...
    with tf.Session() as sess:
        ...
        for i in tqdm(range(test_data_size)):
            ...
            # pre_y_value = sess.run(pre_y, feed_dict=feed_dict)
            # predictions.append(pre_y_value)
