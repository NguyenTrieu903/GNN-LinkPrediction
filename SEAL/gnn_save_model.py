...
def define_model(top_k, initial_channels, nodes_size_list_train, nodes_size_list_test, learning_rate, debug):
...
def train_demo(model, X_train, D_inverse_train, A_tilde_train, Y_train, nodes_size_list_train, epoch):
...
def predict_test_data(model, X_test, D_inverse_test, A_tilde_test, Y_test, nodes_size_list_test):
    ...
    with tf.Session() as sess:
        ...
        Y_pl = graph.get_tensor_by_name("Y-placeholder:0")
...
def train(X_train, D_inverse_train, A_tilde_train, Y_train, nodes_size_list_train,
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epoch):
            ...
            print("After %5s epoch, the loss is %f, training acc %f."
                  % (epoch, loss_value, train_acc))
